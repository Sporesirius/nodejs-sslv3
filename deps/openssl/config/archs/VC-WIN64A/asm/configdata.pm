...
our %config = (
  ...
  openssldir => "",
  options => "enable-ssl-trace enable-ssl3 enable-ssl3-method enable-weak-ssl-ciphers no-afalgeng no-asan no-buildtest-c++ no-comp no-crypto-mdebug no-crypto-mdebug-backtrace no-devcryptoeng no-dynamic-engine no-ec_nistp_64_gcc_128 no-egd no-external-tests no-fuzz-afl no-fuzz-libfuzzer no-heartbeats no-md2 no-msan no-rc5 no-sctp no-shared no-ubsan no-unit-test no-zlib no-zlib-dynamic",
  perl_archname => "x86_64-linux-gnu-thread-multi",
  perl_cmd => "/usr/bin/perl",
  perl_version => "5.26.1",
  perlargv => [ "no-comp", "no-shared", "no-afalgeng", "enable-ssl-trace", "enable-ssl3", "enable-ssl3-method", "enable-weak-ssl-ciphers", "VC-WIN64A" ],
  perlenv => {
      "AR" => undef,
      "ARFLAGS" => undef,
      "AS" => undef,
